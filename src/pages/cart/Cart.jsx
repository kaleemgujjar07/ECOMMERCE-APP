...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    const hashString = `${integritySalt}&${grandTotal}&${merchantId}&${password}&${transactionId}`;
    ...
    const paymentData = {
      ...
      pp_Amount: grandTotal.toString(),
      pp_BillReference: "Order_" + transactionId,
      pp_TxnCurrency: "PKR",
      pp_TxnDateTime: new Date().toISOString(),
      pp_TxnExpiryDateTime: new Date(Date.now() + 10 * 60000).toISOString(),
    ...
    try {
      const response = await axios.post(
        "https://sandbox.jazzcash.com.pk/ApplicationAPI/API/Payment",
        paymentData
      );
      ...
      if (response.data.pp_ResponseCode === "000") {
        ...
        const orderInfo = {
          ...
          email: JSON.parse(localStorage.getItem("user")).user.email,
          userid: JSON.parse(localStorage.getItem("user")).user.uid,
          paymentId: response.data.pp_TxnRefNo,
        };

        const orderRef = collection(fireDB, "order");
        addDoc(orderRef, orderInfo);
      } else {
        toast.error("Payment Failed: " + response.data.pp_ResponseMessage);
      }
    } catch (error) {
            ...
            {cartItems.map((item, index) => (
              <div key={index} className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start">
                ...
                <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  ...
                  <div onClick={() => dispatch(deleteFromCart(item))} className="mt-4 flex sm:mt-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
